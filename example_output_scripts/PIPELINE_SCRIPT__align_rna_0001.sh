...
if [[ ! -e "/data/a2.fq.gz" ]]; then
      echo '[ERROR] Cannot find the following required file, which we expect to exist: </data/a2.fq.gz>
      exit 49
fi
mkdir -p "./Pipeline_01_Spliced_Tophat_Alignment"
tophat -o ./Pipeline_01_Spliced_Tophat_Alignment --min-anchor=5 --segment-length=25 --no-coverage-search --segment-mismatches=2 --splice-mismatches=2 --microexon-search  --GTF=___PLACEHOLDER_FOR_ANNOTATION_FILE___ --num-threads=4   ___PLACEHOLDER_FOR_ANNOTATION_FILE___ /data/a2.fq.gz 
